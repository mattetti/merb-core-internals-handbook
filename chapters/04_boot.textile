...
Merb Bootloader is any Ruby class object that responds to :run (in other words, has class method
run). Here is an example of bootloader from Merb core that initializes Merb logger:
...
h2. MixinSessionContainer bootloader.
...
Before session container can be used, it should be registred. Registration
simply ties container name (memcache, memory, etc) with description and file
that contains module of the container.

MixinSessionContainer bootloader mixes in container module into
Merb::Controller using container name from Merb::Config[:session_store]. If
container name is "none" or "false" or an empty string (that is, set to false or
nil), no session container is mixed in at all.

This bootloader also exits unless Merb::Config[:session_secret_key] is set and
longer than 16 characters. 

Merb::BootLoader::MixinSessionContainer (lib/merb-core/bootloader.rb):
<pre>
